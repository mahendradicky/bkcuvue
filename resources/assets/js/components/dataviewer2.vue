        ...
        <ul class="nav nav-tabs nav-tabs-bottom flex-nowrap mb-0">
          <li class="nav-item">
            <a
              href="#"
              class="nav-link"
              :class="{'active': tabName == 'dasar'}"
              @click.prevent="changeTab('dasar')"
            >
          ...
          <li class="nav-item">
            <a
              href="#"
              class="nav-link"
              :class="{'active': tabName == 'lanjutan'}"
              @click.prevent="changeTab('lanjutan')"
            >
              <i class="icon-folder-search mr-2"></i>
              Pencarian Lanjutan
            </a>
          </li>
        </ul>
                ...
                <div class="row">
                  ...
                  <div class="col-md-3 pb-2" v-if="f.column" v-show="false">
                    <div class="input-group">
                      ...
                      <select
                        class="form-control"
                        @input="selectOperator(f, i, $event)"
                        :disabled="itemDataStat !== 'success'"
                      >
                        <option
            ...
            <div class="row justify-content-between">
                ...
                <div class="row">
                  <div class="col-md-6 pb-2">
                    <button
                      ...
                      @click="applyFilter"
                      :disabled="filterCandidates[0].query_1 == null || filterCandidates[0].query_1 == '' || itemDataStat !== 'success'"
                    >
              ...
              <div class="col-md-7 col-lg-4 text-right">
                <div class="row">
                  ...
                  <div class="col-md-2">
                    <button
                      class="btn bg-orange-300 btn-block"
                      @click="updateOrderDirection"
                      :disabled="itemDataStat !== 'success'"
                    >
            ...
            <div class="row">
              <div class="col-md-12" v-for="(f, i, index) in filterCandidates">
                <div class="row">
                  ...
                  <div class="col-md-1 pb-2">
                    <button
                      class="btn bg-slate-300 btn-block"
                      @click="removeFilter(f,i)"
                      :disabled="itemDataStat !== 'success' || filterCandidates.length == 1"
                    >
              ...
              <div class="col-md-12">
                <button
                  ...
                  :disabled="itemDataStat !== 'success'"
                >
                  <i class="icon-arrow-down5"></i> Tambah Parameter Pencarian
      ...
      <div class="card-header d-print-none" v-if="!isNoButtonRow">
        <div class="row">
          ...
          <div class="col-md-5 col-lg-3 text-right d-none d-sm-block">
            <button
              type="button"
              class="btn bg-blue-300 btn-icon mb-1"
              :disabled="itemDataStat === 'loading'"
              @click.prevent="modalOptionOpen('column')"
              v-if="!isNoKolom"
            >
            ...
            <button
              ...
              :disabled="itemDataStat === 'loading'"
              @click.prevent="modalOptionOpen('excel')"
              v-if="!isNoExcel"
            >
          ...
          <div class="col-md-12 d-block d-sm-none">
            <button
              type="button"
              class="btn bg-blue-300 btn-icon btn-block mb-1"
              :disabled="itemDataStat === 'loading'"
              @click.prevent="modalOptionOpen('column')"
              v-if="!isNoKolom"
            >
      ...
      <div class="table-responsive table-scrollable" style="max-height: 33rem;" :class="tableClass">
        <table class="table table-striped">
          ...
          <thead class="bg-primary">
            <tr class="text-nowrap">
              <th v-for="item in columnData" v-if="!item.hide && !item.disable">
                ...
                <i class="icon-menu-open" v-if="item.name == query.order_column"></i>
              </th>
            </tr>
          </thead>
          ...
          <tbody v-else-if="itemDataStat === 'fail'">
            <tr>
              <td :colspan="columnData.length">Oops.. Terjadi kesalahan, silahkan coba lagi.</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div class="card-footer bg-white d-print-none">
        <div class="row">
          ...
          <div class="col-md-4 pt-2">
            ...
            <!-- desktop -->
            <div v-if="itemDataStat === 'success'" class="d-none d-sm-block">
              Menampilkan {{itemData.from}} -
              {{itemData.to}} entri dari {{itemData.total}} entri
            </div>

            <!-- mobile -->
            <div v-if="itemDataStat === 'success'" class="d-block d-sm-none text-center">
              Menampilkan {{itemData.from}} -
              {{itemData.to}} entri dari {{itemData.total}} entri
            </div>
    ...
    >
      <div slot="modal-body1">
        ...
        <div v-if="modalOptionState === 'column'">
          ...
          <button
            ...
            v-if="column.hide != null && !column.disable"
            @click.prevent="hideColumn(index)"
          >{{column.title}}</button>
          <hr />

          <!-- tutup button -->
          <button class="btn btn-light btn-block" @click.prevent="modalTutup">
        ...
        <div v-else-if="modalOptionState === 'excel'">
          ...
          <!-- download table -->
          <button class="btn btn-light btn-block" @click.prevent="modalExcelOpen('excel')">
            <i class="icon-folder-download2"></i> Download data di tabel
          </button>

          <!-- download all   -->
          <button class="btn btn-light btn-block" @click.prevent="modalExcelOpen('excelAll')">
            <i class="icon-folder-download2"></i> Download semua data
          </button>
          <hr />
          ...
          <button
            class="btn btn-light btn-block"
            v-for="(ex, index) in excelUploads"
            v-if="ex.enabled"
            @click.prevent="modalExcelUploadOpen(index)"
          >
      ...
      <div slot="modal-body2" class="text-center">
        ...
        <div v-else-if="state === 'excelAll'">
          ...
          <div v-if="excelAllDataStat === ''">
            ...
            <button type="button" class="btn btn-warning" @click="modalExcelOk">
              <i class="icon-checkmark5"></i> Ya, download semua
            </button>
          </div>
        ...
        <div v-else-if="state === 'upload'">
          ...
          <!-- info -->
          <div
            class="alert bg-info alert-styled-left mt-2 pt-1 pb-1"
            v-if="excelUploads[excelUploadIndex].info"
          >
            <span class="mb-5 text-semibold">
              <u>Info:</u>
            </span>
          ...
          <input
            type="file"
            class="form-control"
            @change="changeUpload($event.target.files)"
            ref="fileInput"
          />
          ...
          <p class="text-center mt-2">
            ...
            <a href="#" @click.prevent="downloadFormatExcel(excelUploadIndex)">format excel</a>
          </p>
...
export default {
  ...
  mounted() {
    document.addEventListener("keydown", (e) => {
      if (this.isDisable != true) {
        if (e.keyCode == 13) {
          if (
            this.filterCandidates.length > 0 &&
  ...
  methods: {
    ...
    hideColumn(index) {
      ...
      else this.columnData[index].hide = false;
      this.fieldExcel();
    },
    showAllColumn(index) {
    ...
    applyFilter() {
      if (
        ...
        this.filterCandidates[0].query_1 != ""
      ) {
        Vue.set(
          this.$data,
          "appliedFilters",
    ...
    fetchExcelAll() {
      ...
      if (this.query.limit >= this.itemData.total) {
        ...
        axios
          .get("/api/" + this.excelDownloadUrl, { params })
          .then(({ data }) => {
    ...
    uploadExcel(index) {
      ...
      axios
        .post("/api/" + this.excelUploads[index].url, this.files)
        .then((response) => {
    ...
    downloadFormatExcel(index) {
      axios
        .get("/api/download/" + this.excelUploads[index].format_url, {
          responseType: "blob",
        })
        .then((response) => {
