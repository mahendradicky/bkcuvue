<template>
  <div>
    ...
    >
      ...
      <template slot="button-desktop">
        ...
        <router-link
          ...
          v-if="currentUser.can && currentUser.can['create_anggota_cu'] && tipe == 'masih'"
        >
        ...
        <!-- ubah NIK -->
        <button
          @click.prevent="modalConfirmOpen('nik')"
          class="btn btn-light btn-icon mb-1"
          v-if="currentUser.can && currentUser.can['update_anggota_cu']"
        ...
        <!-- ubah identitas -->
        <button
          @click.prevent="ubahData(selectedItem.id,'identitas')"
          class="btn btn-light btn-icon mb-1"
          v-if="currentUser.can && currentUser.can['update_anggota_cu'] && tipe == 'masih'"
          :disabled="!selectedItem.id"
        >
        ...
        <!-- ubah produk -->
        <button
          @click.prevent="ubahData(selectedItem.id,'produk')"
          class="btn btn-light btn-icon mb-1"
          v-if="currentUser.can && currentUser.can['update_anggota_cu'] && tipe == 'masih'"
          :disabled="!selectedItem.id"
        >
        ...
        <!-- klaim jalinan -->
        <button
          @click.prevent="ubahData(selectedItem.nik,'jalinan')"
          class="btn btn-light btn-icon mb-1"
          v-if="currentUser.can && currentUser.can['create_jalinan_klaim'] && tipe == 'masih'"
          :disabled="!selectedItem.nik"
        >
        ...
        <!-- anggota keluar -->
        <button
          @click.prevent="modalConfirmOpen('keluar')"
          class="btn btn-light btn-icon mb-1"
          v-if="currentUser.can && currentUser.can['update_anggota_cu'] && tipe != 'meninggal'"
          :disabled="!selectedItem.id"
        >
        ...
        <button
          ...
          class="btn btn-light btn-icon mb-1"
          v-if="currentUser.can && currentUser.can['destroy_anggota_cu'] && tipe == 'masih'"
          :disabled="!selectedItem.id"
        >
        ...
        <!-- table draft -->
        <router-link
          :to="{ name: 'anggotaCuCuDraft', params:{cu: $route.params.cu, tp: 'semua'} }"
          class="btn btn-light btn-icon mb-1"
          v-if="currentUser && currentUser.can['upload_anggota_cu'] && anggotaCuDraftCountStat == 'success' && anggotaCuDraftCount > 0"
        >
      ...
      <template slot="button-mobile">
        ...
        <router-link
          :to="{ name: kelas + 'Create'}"
          class="btn btn-light btn-icon btn-block pb-1"
          v-if="currentUser.can && currentUser.can['create_anggota_cu'] && tipe == 'masih'"
        >
          <i class="icon-plus3"></i> Tambah
        </router-link>
        ...
        <button
          ...
          v-if="currentUser.can && currentUser.can['create_jalinan_klaim'] && tipe == 'masih'"
          :disabled="!selectedItem.nik || selectedItem.status_jalinan"
        >
          <i class="icon-accessibility2"></i> Ajukan Klaim JALINAN
        </button>
      ...
      <template slot="item-desktop" slot-scope="props">
        ...
        >
          ...
          <td v-if="!columnData[24].hide" v-html="$options.filters.date(props.item.tanggal_lahir)"></td>
    ...
    >
      <!-- title -->
      <template slot="modal-title" v-if="modalState != 'normal1'">{{ modalTitle }}</template>
      ...
      <template slot="modal-body1">
        <div class="text-center">
          ...
          <div v-if="tipe == 'masih'">
            <button
              type="button"
              class="btn btn-primary btn-block pb-1"
              @click="selectCU(cu)"
              v-for="(cu, index) in selectedItem.anggota_cu_cu_not_keluar"
              :key="index"
            >{{ 'CU ' + cu.cu ? cu.cu.name : "-" + ' | ' + 'No. BA: ' + cu.no_ba }}</button>
          ...
          <div v-else>
            <div class="d-none d-sm-block">
              ...
              <button
                ...
                @click="selectCU(cu)"
                v-for="(cu, index) in selectedItem.anggota_cu_cu_keluar"
                :key="index"
              >{{ 'CU ' + cu.cu ? cu.cu.name : "-" + ' | ' + 'No. BA: ' + cu.no_ba }}</button>
            </div>
      ...
      <template slot="modal-body2">
        <form-keluar :anggota_cu="anggota_cu" @tutup="modalTutup"></form-keluar>
      </template>

      <!-- ubah nik -->
      <template slot="modal-body3">
        <form-nik :anggota_cu="anggota_cu" @tutup="modalTutup"></form-nik>
      </template>
    </app-modal>
  </div>
</template>
  ...
  methods: {
    fetch(params) {
      if (this.$route.params.cu == "semua") {
        ...
        if (this.tipe == "masih") {
          ...
          this.$store.dispatch(this.kelas + "/indexCuKeluar", [
            params,
            this.$route.params.cu,
            this.$route.params.tp,
          ]);
          ...
          this.$store.dispatch(this.kelas + "/indexCuMeninggal", [
            params,
            this.$route.params.cu,
            this.$route.params.tp,
          ]);
    ...
    fetchAnggotaCuDraft() {
      ...
      if (this.currentUser.can["upload_anggota_cu"]) {
        axios
          .get("/api/anggotaCuDraft/count/" + cu + "/" + tp)
          .then((response) => {
    ...
    selectCU(value) {
      if (this.state == "keluar") {
        ...
        this.modalTitle =
          "Keluarkan anggota atas nama: " + this.selectedItem.name + " ?";
      ...
      if (state == "hapus") {
        if (this.selectedItem.status_jalinan == null) {
          ...
          this.modalTitle =
            ...
            " tidak bisa dihapus karena memiliki riwayat klaim JALINAN, silahkan periksa kembali lagi.";
        }
      } else if (
        state == "keluar" &&
        this.selectedItem.anggota_cu_cu_not_keluar &&
        this.selectedItem.anggota_cu_cu_not_keluar.length > 1
      ) {
        this.modalState = "normal1";
        this.modalTitle =
          "anggota atas nama: " +
          this.selectedItem.name +
          " memiliki keanggota di beberapa CU, silahkan pilih di CU mana ia akan keluar";
      } else if (
        state == "keluar" &&
        this.selectedItem.anggota_cu_cu_not_keluar &&
        this.selectedItem.anggota_cu_cu_not_keluar.length < 2
      ) {
        this.modalState = "normal2";
        this.modalColor = "bg-primary";
        this.modalTitle =
        ...
        state == "keluar" &&
        this.selectedItem.anggota_cu_cu_keluar &&
        this.selectedItem.anggota_cu_cu_keluar.length < 2
      ) {
        this.modalState = "confirm-tutup";
        this.modalColor = "bg-primary";
        this.modalTitle =
          "Batal keluarkan anggota atas nama: " + this.selectedItem.name + " ?";
        ...
        this.modalColor = "bg-primary";
        this.modalTitle =
          "Ubah NIK anggota atas nama: " + this.selectedItem.name + " ?";
    ...
    modalConfirmOk() {
      if (this.state == "hapus") {
        if (this.$route.params.cu != "semua") {
          this.$store.dispatch(this.kelas + "/destroy", [
